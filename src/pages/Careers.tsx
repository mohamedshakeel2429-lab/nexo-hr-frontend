  ...
  return (
    <div className="pt-32 pb-20">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center glass p-4 rounded-2xl">
          <div className="relative flex-grow w-full">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/5 rounded-xl py-3 pl-12 pr-4 text-white focus:outline-none focus:border-brand-accent transition-colors"
              value={searchTerm}
              ...
              {jobs.length > 0 ? (
                jobs.map((job) => (
                  <motion.div
                    ...
                    key={job._id}
                    className="glass p-8 rounded-3xl hover:border-brand-accent/50 transition-all group flex flex-col justify-between"
                  >
                    <div className="space-y-4">
                      <div className="flex justify-between items-start">
                        <h3 className="text-2xl font-bold text-white group-hover:text-brand-accent transition-colors leading-tight">
                          {job.title}
                        </h3>
                        <span className="px-3 py-1 rounded-full bg-brand-accent/10 text-brand-accent text-[10px] font-black uppercase tracking-widest">
                          {job.type}
                        </span>
                      </div>
                      <div className="flex flex-wrap gap-6 text-sm text-slate-500 font-medium">
                        <div className="flex items-center gap-2"><MapPin size={16} className="text-brand-accent" /> {job.location}</div>
                        <div className="flex items-center gap-2"><Briefcase size={16} className="text-brand-accent" /> {job.category}</div>
                        <div className="flex items-center gap-2"><Clock size={16} className="text-brand-accent" /> {formatDate(job.createdAt)}</div>
                      </div>
                      {job.experienceRange && (
                    ...
                    <div className="mt-8 pt-6 border-t border-white/5 flex justify-between items-center">
                      ...
                      <button
                        ...
                        className="btn-primary px-6 py-3 text-sm uppercase tracking-widest"
                      >
                        Apply Now
                      </button>
                    </div>
                ...
                <div className="lg:col-span-2 text-center py-20 glass rounded-[3rem]">
                  <p className="text-slate-500 text-lg">No matching roles found. Try a different search term.</p>
                </div>
              )}
            </AnimatePresence>
          </div>
        ...
        {selectedJob && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl glass p-8 md:p-12 rounded-[2.5rem] overflow-y-auto max-h-[90vh]"
            >
              ...
              {appState === 'success' ? (
                <div className="py-12 text-center space-y-6">
                  <div className="w-20 h-20 rounded-full bg-brand-accent/20 flex items-center justify-center text-brand-accent mx-auto">
                ...
                <div className="space-y-8">
                  <div className="space-y-2">
                    ...
                    <h2 className="text-3xl font-bold text-white leading-tight">
                      Apply for <br /> {selectedJob.title}
                    </h2>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2 col-span-2 md:col-span-1">
                      ...
                      <input
                        ...
                        type="text"
                        className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-brand-accent transition-all"
                      />
                    </div>
                    <div className="space-y-2 col-span-2 md:col-span-1">
                    ...
                    <div className="space-y-2 col-span-2">
                      ...
                      <textarea
                        ...
                        rows={3}
                        className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-brand-accent transition-all placeholder:text-slate-700"
                    ...
                    <div className="space-y-2 col-span-2">
                      ...
                      <button
                        ...
                        className="w-full border-2 border-dashed border-white/10 rounded-2xl p-8 text-center cursor-pointer hover:bg-white/5 transition-all group"
                      >
                        <Upload className="mx-auto text-slate-600 group-hover:text-brand-accent transition-colors mb-2" />
                        {form.resume ? (
                          ...
                          <p className="text-sm text-slate-500 group-hover:text-slate-300">
                            Drag & drop or <span className="text-brand-accent font-bold underline">browse files</span>
                          </p>
                        )}
                      </button>
                    ...
                    <button
                      ...
                      className="col-span-2 btn-primary py-4 text-lg uppercase tracking-widest mt-4 flex items-center justify-center gap-3 disabled:opacity-50"
                    >
                      {appState === 'submitting' ? (
      ...
      <section className="section-padding text-center">
        <div className="glass p-12 rounded-[3.5rem] max-w-4xl mx-auto space-y-6">
          <h2 className="text-3xl font-bold text-white uppercase tracking-tighter">Don't see your fit?</h2>
          <p className="text-slate-400">
            Send us your CV anyway at <br />
            <a href="mailto:nexo.hrsolutions@gmail.com" className="text-brand-accent font-bold text-lg hover:underline">
          ...
          <div className="flex justify-center pt-4">
            <Link
              to="/contact"
              className="px-8 py-4 rounded-xl glass border-white/5 text-white font-bold flex items-center gap-2 hover:bg-white/10 transition-all uppercase tracking-widest text-sm"
            >
              Inquire about internships <ArrowRight size={18} />
            </Link>
          </div>
        </div>
      </section>
